...
import { z } from "zod";
import { reportSchema, resultsSchema } from "./schema";
...
function result<T extends z.ZodTypeAny>(schema: T) {
  return z.union([
    z.intersection(successResultSchema, schema),
    errorResultSchema,
  ]);
}
  ...
  exportCsv: {
    ...
    args: z.object({
      fileName: z
        .string()
        .describe(
          "The file name for the exported CSV file. Must end in `.csv`."
        ),
      sql: z.string().describe(codeBlock`
 ...
 * Duplicated since this is not exported by their lib.
 */
export type ToolResult<Name extends string, Args, Result> = ToolCall<
  Name,
  Args
> & {
  result: Result;
